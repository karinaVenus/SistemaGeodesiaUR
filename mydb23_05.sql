...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 23-05-2022 a las 08:03:24
...
-- Procedimientos
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `kardex_articulo` (IN `articulo` VARCHAR(12), IN `fec_ini` DATE, IN `fec_fin` DATE)  select art.cod_art, 'INGRESO' as movimiento,
    Date_format(ri.fec_ing,'%d/%M/%Y') as FECHA, rid.cant_art as CANT_ING, rid.prec_unit as PREC_ING, rid.prec_unit*rid.cant_art as VAL_ING, '' as CANT_SAL, '' as PREC_SAL, '' as VAL_SAL
    ...
    where art.cod_art = articulo && ri.fec_ing BETWEEN CONCAT(fec_ini," 00:00:00") and CONCAT(fec_fin," 23:59:59")
    union
    select art.cod_art, 'SALIDA' as movimiento,
    Date_format(rs.fec_sal,'%d/%M/%Y') as FECHA, '' as CANT_ING, '' as PREC_ING,'' as VAL_ING, rsd.cant_art as CANT_SAL, rsd.prec_sal as PREC_SAL, rsd.prec_sal*rsd.cant_art as VAL_SAL
    from articulo as art
    left join reg_sal_det as rsd on rsd.cod_art = art.cod_art
    inner join reg_sal_cab as rs on rs.cod_reg_sal = rsd.cod_reg_sal
    where art.cod_art = articulo  && rs.fec_sal BETWEEN CONCAT(fec_ini," 00:00:00") and CONCAT(fec_fin," 23:59:59")
    order by FECHA asc$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `total_cant_lapso` (IN `articulo` VARCHAR(12), IN `fec_ini` DATE)  select(
(SELECT 
sum(cant_art) as CANT FROM reg_ing_aux WHERE cod_art = articulo)
-
(select sum(rid.cant_art) as CANT 
from articulo as art
right join reg_ing_det as rid on rid.cod_art = art.cod_art
inner join reg_ing_cab as ri on ri.cod_reg_in = rid.cod_reg_ing 
where art.cod_art = articulo && ri.fec_ing BETWEEN CONCAT(fec_ini," 00:00:00") and NOW())
+
(select sum(rsd.cant_art) as CANT 
from articulo as art
left join reg_sal_det as rsd on rsd.cod_art = art.cod_art
inner join reg_sal_cab as rs on rs.cod_reg_sal = rsd.cod_reg_sal
where art.cod_art = articulo  && rs.fec_sal BETWEEN CONCAT(fec_ini," 00:00:00") and NOW() )
) as cant_ini$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `total_valor_lapso` (IN `articulo` VARCHAR(12), IN `fec_ini` DATE)  begin
select(
(SELECT 
sum(prec_art*cant_art) as TOTAL FROM reg_ing_aux WHERE cod_art = articulo)
 -
(select sum(rid.prec_unit*rid.cant_art) as TOTAL
from articulo as art
right join reg_ing_det as rid on rid.cod_art = art.cod_art
inner join reg_ing_cab as ri on ri.cod_reg_in = rid.cod_reg_ing 
where art.cod_art = articulo && ri.fec_ing BETWEEN CONCAT(fec_ini," 00:00:00") and NOW())
+
(select  sum(rsd.prec_sal*rsd.cant_art) as TOTAL
from articulo as art
left join reg_sal_det as rsd on rsd.cod_art = art.cod_art
inner join reg_sal_cab as rs on rs.cod_reg_sal = rsd.cod_reg_sal
where art.cod_art = articulo && rs.fec_sal BETWEEN CONCAT(fec_ini," 00:00:00") and NOW() )
...
('ART02', 'cinta metrica', 2, 1, 1, 22, 'img1.jpg', 1),
('ART03', 'sierra electrica', 1, 1, 1, 42, 'img3.jpg', 1),
('ART04', 'cartulina de correspum', 1, 3, 1, 1, 'img4.jpg', 1),
...
INSERT INTO `reg_ing_aux` (`fec_ing`, `cod_art`, `prec_art`, `cant_art`) VALUES
('2022-04-27 17:33:48', 'ART01', '60.00', 7),
('2022-04-27 17:33:48', 'ART03', '45.00', 5),
('2022-05-18 17:52:12', 'ART01', '60.00', 34),
('2022-05-18 17:52:12', 'ART02', '30.00', 21),
('2022-05-18 17:52:12', 'ART03', '45.00', 23);
...
(31, 3, 1, 1, 2, 1, '121323', '2022-05-01', '2022-04-23 17:33:48', 1, '775.00'),
(32, 3, 1, 1, 2, 1, '343245', '2022-05-05', '2022-04-27 17:33:48', 1, '1315.00'),
(33, 3, 2, 1, 2, 1, '34546', '2022-05-26', '2022-05-16 17:52:12', 1, '1750.00');
...
(31, 'ART01', '25.00', 20, '250.00', 'OBS1'),
(31, 'ART02', '30.00', 10, '300.00', 'OBS2'),
(31, 'ART03', '45.00', 5, '225.00', 'OBS3'),
(32, 'ART01', '60.00', 10, '600.00', 'OBS4'),
(32, 'ART02', '50.00', 8, '400.00', 'OBS5'),
(32, 'ART03', '45.00', 7, '315.00', 'OBS6'),
(33, 'ART01', '60.00', 34, '2040.00', NULL),
(33, 'ART02', '30.00', 23, '690.00', NULL),
(33, 'ART03', '45.00', 23, '1035.00', NULL);
...
CREATE TRIGGER `tr_añadir_aux` AFTER INSERT ON `reg_ing_det` FOR EACH ROW BEGIN
    ...
    SELECT fec_ing into @fecha FROM reg_ing_cab as ric WHERE ric.cod_reg_in = new.cod_reg_ing;
  	...
  	INSERT INTO reg_ing_aux VALUES (@fecha, new.cod_art, new.prec_unit, new.cant_art);
END
$$
DELIMITER ;
...
(91, 1, 2, 2, 3, 4, '11233245', '2022-05-18', '2022-05-12 17:47:55', 1, '1500.00'),
(92, 1, 2, 2, 3, 4, '11233245', '2022-05-18', '2022-05-12 17:49:41', 1, '1500.00'),
(93, 1, 2, 2, 3, 4, '11233245', '2022-05-18', '2022-05-21 17:55:29', 1, '1500.00');
...
(91, 'ART01', 11, '25.00', 'ObsSal1'),
(91, 'ART02', 10, '30.00', 'ObsSal1'),
(91, 'ART02', 2, '50.00', 'ObsSal1'),
(91, 'ART03', 7, '45.00', 'BOSAs'),
(92, 'ART01', 4, '25.00', 'ObsSal1'),
(92, 'ART02', 3, '50.00', 'ObsSal1'),
(93, 'ART01', 5, '25.00', 'ObsSal1'),
(93, 'ART01', 3, '60.00', 'ObsSal1'),
(93, 'ART02', 3, '50.00', 'ObsSal1'),
(93, 'ART02', 2, '30.00', 'ObsSal1');
...
--
ALTER TABLE `reg_ing_cab`
  MODIFY `cod_reg_in` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=34;

--
-- AUTO_INCREMENT de la tabla `reg_sal_cab`
--
ALTER TABLE `reg_sal_cab`
  MODIFY `cod_reg_sal` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=94;
